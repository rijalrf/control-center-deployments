...
const PAGE_TITLES: Record<string, { title: string; subtitle: string }> = {
  ...
  '/deployment':    { title: 'Deployment',      subtitle: 'Orchestrate remote deployments' },
  '/configuration': { title: 'Configuration',  subtitle: 'Environments & server management' },
}
  ...
  return (
    <header className="h-16 shrink-0 bg-ccd-surface border-b border-ccd-border px-6 flex items-center justify-between">
      ...
      <div className="flex items-center gap-4">
        {/* Status indicator */}
        <div className="flex items-center gap-2 text-xs text-ccd-text-muted">
          <span className="w-2 h-2 rounded-full bg-ccd-success animate-pulse" />
        ...
        {user && (
          <div className="flex items-center gap-3">
            <div className="text-right hidden sm:block">
              <div className="text-sm font-medium text-ccd-text">{user.name || user.login}</div>
